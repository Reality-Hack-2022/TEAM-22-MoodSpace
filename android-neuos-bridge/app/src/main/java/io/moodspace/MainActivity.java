...
public class MainActivity extends AppCompatActivity {
    ...
    final String TAG = "MoodSpace";

    // TODO: This API key should be elsewhere
    final String API_KEY = "jP4gzbRwyUDAgKkvf";
    ...
    private MoodSpaceSession mSession;

    static final String DEFAULT_USER_NAME = "UserAnon";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_moodspace);
        ...
        // Session init
        mSession = MoodSpaceSession.create(mDatabase);
    ...
    public void onStartSession(View view) {
        ...
        String userName = MainActivity.DEFAULT_USER_NAME;
        if (this.userName.getText().toString() != "") {
            userName = this.userName.getText().toString();
        }

        mSession.agent = userName;
        mSession.startTime = timeStampMillis;

        DatabaseReference session = mDatabase.child("sessions").child(mSession.id);
        session.child("agent").setValue(mSession.agent);
        session.child("startTime").setValue(timeStampMillis);
    }
    ...
    private final INeuosSdkListener mCallback = new INeuosSdkListener.Stub() {
        ...
        public void onValueChanged(String key, float value) throws RemoteException {
            ...
            Instant instant = Instant.now();
            String timeStampMillis = String.valueOf(instant.toEpochMilli());
            mDatabase.child("sessions").child(mSession.id).child(key).child(timeStampMillis).setValue(value);
        }
        @Override
